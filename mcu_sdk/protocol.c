...
void uart_transmit_output(unsigned char value)
{
    ...
    extern void Uart1_PutChar(unsigned char value);
    Uart1_PutChar(value);
/*
    //Example:
...
void all_data_update(void)
{
   // #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    mcu_dp_value_update(DPID_TEMP_CURRENT,FlashBuffer.temper); //VALUE型 温度数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,FlashBuffer.humidity); //VALUE型 湿度数据上报;
    mcu_dp_value_update(DPID_TEMP_SAMPLING,FlashBuffer.SAMPLING); //VALUE型 温度采样时间数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,FlashBuffer.SAMPLING); //VALUE型 湿度采样时间数据上报;
    ...
    mcu_dp_bool_update(DPID_LED_CONNECT,当前网络连接指示灯); //BOOL型数据上报;
